...
;;On key event "d" apen-down event occurs.
;;On key- event "u" the pen is pulled up.
...
;;Example:
;;(draw-normal-rect RECT-1)
;;(draw-normal-rect RECT-2)
...
;;print-locus :ListOfPoints->Point
...
;; rect-after-drag : Rectangle Integer Integer Integer Integer  Rectangle-> Rectangle.
...
;;rects-after-drag :RectangleList Int Int PosInt PosInt  RectangleList-> RectangleList.
...
;; if the rectangle is selected, return a rectangle just like the given one,
;; except that it is no longer selected.
;; STRATEGY: Use template on rects rl.
;;;Examples:
...
;; rects-after-button-up : Rectanglelist -> Rectanglelist.
...
(define (rect-after-key-event r kev)
  (cond
    ...
    [(key=? kev "d")(make-rect (rect-x r) (rect-y r)
                               (rect-vx r)(rect-vy r) true true)]
    [(key=? kev "u")(make-rect (rect-x r) (rect-y r)
                               (rect-vx r)(rect-vy r) true false)]
    [else r]
    ) )
